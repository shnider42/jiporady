const STORAGE_PREFIX = "jiporady_state_v2";
...
function money(value) {
    ...
    return `${state.score < 0 ? "-" : ""}$${Math.abs(state.score).toLocaleString()}`;
}
...
function updateScorePanel() {
    ...
    if (state.activeRound === "final") {
        progressEl.textContent = `${solvedCluesAcrossRounds()} / ${totalCluesAcrossRounds()}`;
        return;
    }
...
function setGameData(data, resetProgress = false) {
    gameData = data;
    storageKey = `${STORAGE_PREFIX}:${gameData.board_id}`;
    state = resetProgress ? defaultState() : loadState();
    saveState();
    renderBoard();
}
...
newGameBtn.addEventListener("click", async () => {
    const confirmed = window.confirm("Start a fresh randomized board and reset score/progress?");
    if (!confirmed) {
        return;
    }
    ...
    try {
        ...
        console.error(error);
        window.alert("Could not start a new game.");
    }
});

clearStorageBtn.addEventListener("click", () => {
    const confirmed = window.confirm("Hard reset the saved progress for this board in this browser?");
    if (!confirmed) {
        return;
    }
